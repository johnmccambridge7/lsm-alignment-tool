...
class Application(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        self.title_label = ctk.CTkLabel(self.main_frame, text="ClariTir: Z-Stack Formatter Suite", font=("Arial", 24))
        self.title_label.place(relx=0.5, rely=0.35, anchor='center')  # Adjust this value so it's slightly above center
        ...
        credit_label = ctk.CTkLabel(self.main_frame, text="Copyright: Penelope L. Tir, John L. McCambridge, 2023", font=("Arial", 10), text_color=("black", "#606363"))
    ...
    def save_image(self):
        ...
        new_image = np.array([self.processed_channels[x] for x in range(len(self.processed_channels.keys()))])
        new_image = new_image.transpose((1, 0, 2, 3))

        #re-organizing so that the shape matches the input shape
        """for _, channels in enumerate(zip(*self.processed_channels)):
            new_image.append(channels)
        ...
        pixels_per_micrometer = 1.6091
        dpi = pixels_per_micrometer # int(pixels_per_micrometer * 25400)
